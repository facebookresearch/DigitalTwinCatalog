...
class SdfRenderer(nn.Module):
    ...
    def update_occupancy_grid(self, plane_xy, plane_xz, plane_yz, triplane):
        def occ_eval_fn(x):
            ...
            for n in range(0, chunk_num):
                ...
                alpha = self.get_alpha(sdf, self.render_step_size)
    ...
    def get_alpha(self, sdf, dists):
        density = self.inv_std * (
            0.5 + 0.5 * sdf.sign() * torch.expm1(-sdf.abs() * self.inv_std)
        )
    ...
    def evaluate_points_sdf(
        ...
        sdf = torch.cat(sdf_arr, dim=0).reshape(batch_size, point_num, 1)
        return sdf
    ...
    def sample_point(
        ...
        out = {}
        out["sdf"] = self.evaluate_points_sdf(
    ...
    def forward(
        ...
        alpha = self.get_alpha(sdf, t_intervals)
