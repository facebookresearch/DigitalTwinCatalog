...
__global__ void infer_t_minmax_cuda_kernel(
  ...
  if (i_ray < n_rays) {
    ...
    float _t_min =
        max(min(max(max(min(ax, bx), min(ay, by)), min(az, bz)), far), near);
    float _t_max =
        max(min(min(min(max(ax, bx), max(ay, by)), max(az, bz)), far), near);
    float valid = static_cast<float>((_t_max >= _t_min));
    t_min[i_ray] = _t_min * valid;
    t_max[i_ray] = _t_max * valid;
  }
}
...
__global__ void infer_n_samples_cuda_kernel(
  ...
  if (i_ray < n_rays) {
    ...
    // at least 1 point for easier implementation in the later
    // sample_pts_on_rays_cuda
    n_samples[i_ray] =
        max(ceil((t_max[i_ray] - t_min[i_ray]) * rnorm / stepdist), 1.);
  }
}
...
__global__ void infer_ray_start_dir_cuda_kernel(
  ...
  if (i_ray < n_rays) {
    ...
    rays_start[offset + 1] =
        rays_o[offset + 1] + rays_d[offset + 1] * t_min[i_ray];
    rays_start[offset + 2] =
        rays_o[offset + 2] + rays_d[offset + 2] * t_min[i_ray];
...
std::vector<torch::Tensor> infer_t_minmax_cuda(
  ...
  infer_t_minmax_cuda_kernel<float_t><<<blocks, threads>>>(
      rays_o.data<float_t>(),
      rays_d.data<float_t>(),
      xyz_min.data<float_t>(),
      xyz_max.data<float_t>(),
      near,
      far,
      n_rays,
      t_min.data<float_t>(),
...
torch::Tensor infer_n_samples_cuda(
  ...
  auto n_samples =
      torch::empty({n_rays}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  const int threads = 256;
  const int blocks = (n_rays + threads - 1) / threads;
  infer_n_samples_cuda_kernel<float_t><<<blocks, threads>>>(
      rays_d.data<float_t>(),
      t_min.data<float_t>(),
      t_max.data<float_t>(),
      stepdist,
...
std::vector<torch::Tensor> infer_ray_start_dir_cuda(
  ...
  infer_ray_start_dir_cuda_kernel<float_t><<<blocks, threads>>>(
      rays_o.data<float_t>(),
      rays_d.data<float_t>(),
      t_min.data<float_t>(),
      n_rays,
      rays_start.data<float_t>(),
      rays_dir.data<float_t>());
...
std::vector<torch::Tensor> sample_pts_on_rays_cuda(
  ...
  auto t_minmax =
      infer_t_minmax_cuda(rays_o, rays_d, xyz_min, xyz_max, near, far);
  ...
  auto ray_id = torch::zeros(
      {total_len}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  ...
  auto rays_pts = torch::empty(
      {total_len, 3}, torch::dtype(rays_o.dtype()).device(torch::kCUDA));

  sample_pts_on_rays_cuda_kernel<float_t>
      <<<(total_len + threads - 1) / threads, threads>>>(
          rays_start.data<float_t>(),
          rays_dir.data<float_t>(),
          xyz_min.data<float_t>(),
          xyz_max.data<float_t>(),
          ...
          stepdist,
          total_len,
          rays_pts.data<float_t>());
  return {rays_pts, ray_id, step_id, N_steps, t_min, t_max};
}

template <typename scalar_t>
__device__ __forceinline__ scalar_t
norm3(const scalar_t x, const scalar_t y, const scalar_t z) {
...
__global__ void sample_bg_pts_on_rays_cuda_kernel(
  ...
  if (idx < N_samples * n_rays) {
    ...
    ori_ray_pts_outer = (rays_o[:,None,:] + rays_d[:,None,:] *
    ori_t_outer[:,:,None]).reshape(-1,3) t_outer =
    ori_ray_pts_outer.norm(dim=-1) R_outer = t_outer /
    ori_ray_pts_outer.abs().amax(1) # r = R * R / t o2i_p = R_outer.pow(2) /
    t_outer.pow(2) * (1-self.bg_preserve) + R_outer / t_outer * self.bg_preserve
    ray_pts_outer = (ori_ray_pts_outer * o2i_p[:,None]).reshape(len(rays_o), -1,
    3)
   */
    const float t_inner = t_max[i_ray];
    const float ori_t_outer =
        t_inner - 1. + 1. / (1. - ((float)i_step) / N_samples);
    const float ori_ray_pts_x =
        rays_o[offset_r] + rays_d[offset_r] * ori_t_outer;
    const float ori_ray_pts_y =
        rays_o[offset_r + 1] + rays_d[offset_r + 1] * ori_t_outer;
    const float ori_ray_pts_z =
        rays_o[offset_r + 2] + rays_d[offset_r + 2] * ori_t_outer;
    const float t_outer = norm3(ori_ray_pts_x, ori_ray_pts_y, ori_ray_pts_z);
    const float ori_ray_pts_m =
        max(abs(ori_ray_pts_x), max(abs(ori_ray_pts_y), abs(ori_ray_pts_z)));
    const float R_outer = t_outer / ori_ray_pts_m;
    const float o2i_p =
        R_outer * R_outer / (t_outer * t_outer) * (1. - bg_preserve) +
        R_outer / t_outer * bg_preserve;
...
torch::Tensor sample_bg_pts_on_rays_cuda(
  ...
  auto rays_pts = torch::empty(
      {n_rays, N_samples, 3},
      torch::dtype(rays_o.dtype()).device(torch::kCUDA));

  sample_bg_pts_on_rays_cuda_kernel<float_t>
      <<<(n_rays * N_samples + threads - 1) / threads, threads>>>(
          rays_o.data<float_t>(),
          rays_d.data<float_t>(),
          t_max.data<float_t>(),
          bg_preserve,
...
check_xyz(int i, int j, int k, int sz_i, int sz_j, int sz_k) {
  return (0 <= i) && (i < sz_i) && (0 <= j) && (j < sz_j) && (0 <= k) &&
      (k < sz_k);
}

template <typename scalar_t>
__global__ void maskcache_lookup_cuda_kernel(
    bool* __restrict__ maskgrid,
  ...
  if (i_pt < n_pts) {
    ...
    if (check_xyz(i, j, k, sz_i, sz_j, sz_k)) {
      out[i_pt] = maskgrid[i * sz_j * sz_k + j * sz_k + k];
    }
  }
}
...
torch::Tensor maskcache_lookup_cuda(
  ...
  const int sz_i = maskgrid.size(0);
  const int sz_j = maskgrid.size(1);
  const int sz_k = maskgrid.size(2);
  const int n_pts = xyz.size(0);

  auto out =
      torch::zeros({n_pts}, torch::dtype(torch::kBool).device(torch::kCUDA));
  if (n_pts == 0) {
    return out;
  }
  ...
  maskcache_lookup_cuda_kernel<float_t><<<blocks, threads>>>(
      maskgrid.data<bool>(),
      xyz.data<float_t>(),
      out.data<bool>(),
      xyz2ijk_scale.data<float_t>(),
      xyz2ijk_shift.data<float_t>(),
      sz_i,
      sz_j,
      sz_k,
      n_pts);
...
template <typename scalar_t>
__global__ void maskcache_ray_tracing_cuda_kernel(
    bool* __restrict__ maskgrid,
    scalar_t* __restrict__ rs,
    scalar_t* __restrict__ rd,
    const float stepdist,
    int64_t* __restrict__ max_n_steps,
    scalar_t* __restrict__ xyz2ijk_scale,
    scalar_t* __restrict__ xyz2ijk_shift,
    const int sz_i,
    const int sz_j,
    const int sz_k,
    const int n_rays,
    int64_t* __restrict__ rt_n_steps) {
  const auto i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_ray < n_rays) {
    const int offset = i_ray * 3;
    scalar_t x = rs[offset] * xyz2ijk_scale[0] + xyz2ijk_shift[0];
    scalar_t y = rs[offset + 1] * xyz2ijk_scale[1] + xyz2ijk_shift[1];
    scalar_t z = rs[offset + 2] * xyz2ijk_scale[2] + xyz2ijk_shift[2];
    const scalar_t dx = rd[offset] * stepdist * xyz2ijk_scale[0];
    const scalar_t dy = rd[offset + 1] * stepdist * xyz2ijk_scale[1];
    const scalar_t dz = rd[offset + 2] * stepdist * xyz2ijk_scale[2];

    int ns;
    const int max_n = max_n_steps[i_ray];
    for (ns = 0; ns < max_n; ++ns, x += dx, y += dy, z += dz) {
      const int i = round(x);
      const int j = round(y);
      const int k = round(z);
      if (check_xyz(i, j, k, sz_i, sz_j, sz_k) &&
          maskgrid[i * sz_j * sz_k + j * sz_k + k]) {
        break;
      }
    }
    rt_n_steps[i_ray] = ns;
...
torch::Tensor maskcache_ray_tracing_cuda(
  ...
  auto rt_n_steps = torch::zeros_like(max_n_steps);
  // max_n_steps.clone();
  if (n_rays == 0) {
    return rt_n_steps;
  }
  ...
  maskcache_ray_tracing_cuda_kernel<float_t><<<blocks, threads>>>(
      maskgrid.data<bool>(),
      rs.data<float_t>(),
      rd.data<float_t>(),
      stepdist,
      max_n_steps.data<int64_t>(),
      xyz2ijk_scale.data<float_t>(),
      xyz2ijk_shift.data<float_t>(),
      sz_i,
      sz_j,
...
raw2alpha_cuda(torch::Tensor density, const float shift, const float interval) {
  ...
  raw2alpha_cuda_kernel<float_t><<<blocks, threads>>>(
      density.data<float_t>(),
      shift,
      interval,
      n_pts,
      exp_d.data<float_t>(),
...
std::vector<torch::Tensor> raw2alpha_nonuni_cuda(
  ...
  raw2alpha_nonuni_cuda_kernel<float_t><<<blocks, threads>>>(
      density.data<float_t>(),
      shift,
      interval.data<float_t>(),
      n_pts,
      exp_d.data<float_t>(),
      alpha.data<float_t>());
...
__global__ void raw2alpha_backward_cuda_kernel(
  ...
  const auto i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_pt < n_pts) {
    grad[i_pt] = min(exp_d[i_pt], 1e10) * pow(1 + exp_d[i_pt], -interval - 1) *
        interval * grad_back[i_pt];
  }
}
...
__global__ void raw2alpha_nonuni_backward_cuda_kernel(
  ...
  const auto i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_pt < n_pts) {
    grad[i_pt] = min(exp_d[i_pt], 1e10) *
        pow(1 + exp_d[i_pt], -interval[i_pt] - 1) * interval[i_pt] *
        grad_back[i_pt];
  }
}
...
torch::Tensor raw2alpha_backward_cuda(
  ...
  raw2alpha_backward_cuda_kernel<float_t><<<blocks, threads>>>(
      exp_d.data<float_t>(),
      grad_back.data<float_t>(),
      interval,
      n_pts,
      grad.data<float_t>());
...
torch::Tensor raw2alpha_nonuni_backward_cuda(
  ...
  raw2alpha_nonuni_backward_cuda_kernel<float_t><<<blocks, threads>>>(
      exp_d.data<float_t>(),
      grad_back.data<float_t>(),
      interval.data<float_t>(),
      n_pts,
...
alpha2weight_cuda(torch::Tensor alpha, torch::Tensor ray_id, const int n_rays) {
  ...
  auto i_start =
      torch::zeros({n_rays}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  auto i_end =
      torch::zeros({n_rays}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  if (n_pts == 0) {
  ...
  alpha2weight_cuda_kernel<float_t><<<blocks, threads>>>(
      alpha.data<float_t>(),
      n_rays,
      weight.data<float_t>(),
      T.data<float_t>(),
      alphainv_last.data<float_t>(),
      i_start.data<int64_t>(),
      i_end.data<int64_t>());
...
__global__ void alpha2weight_backward_cuda_kernel(
  ...
  if (i_ray < n_rays) {
    ...
    float back_cum = grad_last[i_ray];
    for (int i = i_e - 1; i >= i_s; --i) {
      grad[i] = T[i] * (grad_weights[i] - back_cum);
      back_cum = back_cum * (1 - alpha[i]) + grad_weights[i] * alpha[i];
    }
  }
}
...
torch::Tensor alpha2weight_backward_cuda(
  ...
  alpha2weight_backward_cuda_kernel<float_t><<<blocks, threads>>>(
      alpha.data<float_t>(),
      weight.data<float_t>(),
      T.data<float_t>(),
      alphainv_last.data<float_t>(),
      ...
      i_end.data<int64_t>(),
      n_rays,
      grad_weights.data<float_t>(),
      grad_last.data<float_t>(),
      grad.data<float_t>());
 ...
 */
template <typename scalar_t>
__global__ void aggregate_tensorf_val_cuda_kernel(
    scalar_t* __restrict__ xy_feat,
    scalar_t* __restrict__ z_feat,
    scalar_t* __restrict__ xz_feat,
    scalar_t* __restrict__ y_feat,
    scalar_t* __restrict__ yz_feat,
    scalar_t* __restrict__ x_feat,
    int n_pts,
    int n_dim,
    scalar_t* __restrict__ feat) {
  const auto i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_pt < n_pts) {
    scalar_t accum = 0;
    for (int i = 0, j = i_pt; i < n_dim; ++i, j += n_pts) {
      accum += xy_feat[j] * z_feat[j] + xz_feat[j] * y_feat[j] +
          yz_feat[j] * x_feat[j];
    }
    feat[i_pt] = accum;
  }
...
torch::Tensor aggregate_tensorf_val_cuda(
    ...
    torch::Tensor x_feat) {
  const int n_pts = xy_feat.size(1);
  const int n_dim = xy_feat.size(0);
  auto feat = torch::empty({n_pts}, xy_feat.options());
  const int threads = 256;
  const int blocks = (n_pts + threads - 1) / threads;
  aggregate_tensorf_val_cuda_kernel<float_t><<<blocks, threads>>>(
      xy_feat.data<float_t>(),
      z_feat.data<float_t>(),
      xz_feat.data<float_t>(),
      y_feat.data<float_t>(),
      yz_feat.data<float_t>(),
      x_feat.data<float_t>(),
      n_pts,
      n_dim,
      feat.data<float_t>());
 ...
 */
template <typename scalar_t>
__global__ void alpha2weight_dense_cuda_kernel(
    scalar_t* __restrict__ alpha,
    const int n_ray,
    const int n_step,
    scalar_t* __restrict__ weight,
  ...
  const auto i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_ray < n_ray) {
    const int i_start = i_ray * n_step;
    const int i_end = i_start + n_step;
    scalar_t T_cum = 1.;
...
std::vector<torch::Tensor> alpha2weight_dense_cuda(torch::Tensor alpha) {
  const int n_ray = alpha.size(0);
  const int n_step = alpha.size(1);

  auto weight = torch::zeros_like(alpha);
  auto T = torch::ones_like(alpha);
  auto alphainv_last = torch::ones({n_ray}, alpha.options());

  const int threads = 256;
  const int blocks = (n_ray + threads - 1) / threads;

  alpha2weight_dense_cuda_kernel<float_t><<<blocks, threads>>>(
      alpha.data<float_t>(),
      n_ray,
      n_step,
      weight.data<float_t>(),
  ...
  return {weight, T, alphainv_last};
}

template <typename scalar_t>
__global__ void alpha2weight_dense_backward_cuda_kernel(
    scalar_t* __restrict__ alpha,
    scalar_t* __restrict__ weight,
    scalar_t* __restrict__ T,
    scalar_t* __restrict__ alphainv_last,
  ...
  const auto i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if (i_ray < n_ray) {
    const int i_start = i_ray * n_step + n_step - 1;
    const int i_end = i_start - n_step;
    ...
    for (int i = i_start; i > i_end; --i) {
      ...
      back_cum = fma(grad_weights[i], weight[i], back_cum);
    }
  }
}
...
torch::Tensor alpha2weight_dense_backward_cuda(
  ...
  alpha2weight_dense_backward_cuda_kernel<float_t><<<blocks, threads>>>(
      alpha.data<float_t>(),
      weight.data<float_t>(),
      T.data<float_t>(),
      alphainv_last.data<float_t>(),
